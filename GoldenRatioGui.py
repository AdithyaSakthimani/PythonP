...
class EmployeeData:
    def _init_(self, master):                 #Python constructor is a special class method for creating and initializing an object instance at that class.
        self.master = master
    ...
    def add_employee(self):
        add_window = tk.Toplevel(self.master) #it is used to create additional top window or child window
    ...
    def confirm_add_employee(self, emp_id, name, dob, post, add_window):
        if emp_id and name and dob and post:
            employee_info = f"      Name: {name}      DOB: {dob}        Position: {post}" #f strings
    ...
    def confirm_delete_employee(self, emp_id, delete_window):
        if emp_id in self.employee_data:
            ...
            self.display_text.insert(tk.END, f"Employee ID {emp_id} deleted.\n") #used to represent the end of the text, so it is used to add at the end of the text
        else:
    ...
    def display_employees(self):
        ...
        if self.employee_data:
            ...
            messagebox.showinfo("No emoloyee found",f"Employee data is not found")


if _name_ == "_main_":         #checks if the script is being run as the main program. It is not being imported as a module
    root = tk.Tk()
